...
matrix = matrix.map(function(value) {
    var row = value.match(/\S+/g).map(function(cell){
        return parseInt(cell)
    });
    return bubble_sort(row);
})

matrix.forEach(function(value) {
    var smallest_number = value[0];
    var largest_number = value[value.length - 1];
    answer += (largest_number - smallest_number);
});
